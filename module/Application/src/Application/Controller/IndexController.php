...
class IndexController extends AbstractActionController {
    ...
    public function indexAction() {
        ...
        $em = $this->getEntityManager();
		// change by Shehan@ITSthe1 - 28012016
        ...
        if ($request->isPost()) {
            ...
            if ($signupForm->isValid()) {
                $checkIfEmailAlreadyExist = $em->getRepository('\Admin\Entity\Users')->checkIfEmailAlreadyExist($data['email']);
                if (empty($checkIfEmailAlreadyExist)) {
					...
					$dobDate = date_create(date('Y-m-d', strtotime('-18 year')));
                    $userObj->setDateOfBirth($dobDate);
        ...
        if ($city == "") {
			...
			// change by Shehan@ITSthe1 - 28012016
            //$cityobj = $em->getRepository('Admin\Entity\City')->find($city);
			// only for the time being we will use Dubai as the default city
              $cityobj = $em->getRepository("Admin\Entity\City")->findBy(array('cityName' => array("Dubai", "Abu Dhabi", "Doha")));
			// end change
        }
        if (!empty($cityobj)) {
    ...
    /**
     * Update to ajax call - by Yesh - 26012016
     * @return string 1|0 
     */
    public function checkemailAction() {
        ...
        if ($request->isPost()) {
            ...
            $email = $data['email'];
            if (empty($email)) {
                print json_encode(array('status' => 'success', 'result' => 2));
                die();
            } else {
                $em = $this->getEntityManager();
                $checkIfEmailExist = $em->getRepository('\Admin\Entity\Users')->checkIfEmailAlreadyExist($email);
                if (!empty($checkIfEmailExist)) {
                    print json_encode(array('status' => 'success', 'result' => 1)); /* Means Email Exists */
                    die();
                }
            }
            print json_encode(array('status' => 'success', 'result' => 0));
            die();
        } else {
...
//        return $this->getResponse()->setContent("0");
    ...
    public function ajaxsignupAction() {
        ...
        if ($request->isPost()) {
            $data = $this->getRequest()->getPost(); /* Fetch Post Variables */
            parse_str($data['formData'], $formData); //string array
            $formValidator = new Forms\Validator\SignupFormValidator();
            /* Sign up Form Validator Object */
            $signupForm->setInputFilter($formValidator->getInputFilter()); /* Apply filter to form */
            $signupForm->setData($formData); /* set post data to form */
            // check if form is valid
            if ($signupForm->isValid()) {
                ...
                if (empty($checkIfEmailAlreadyExist)) {
                    ...
                    $userObj->setPassword(md5($formData['password']));
                    $userObj->setStatus(1);
                    $userObj->setFbUser(0);
                    $userObj->setUserType("N");
                    $userObj->setCreatedDate($currentDate);
                    ...
                    $commonPlugin = $this->Common();
                    $web_url = $commonPlugin->getBasePathOfProj();
                    $arrMacros = array('$FIRSTNAME' => $formData['fname'], '$LASTNAME' => '', '$EMAIL' => $formData['email'], '$PASSWORD' => $formData['password'], '$WEBURL' => $web_url);
                    $emailSent = $commonPlugin->sendEmail($arrMacros, 'admin_register_user', $userObj->getId());
                    $succsMsg = "You have signed up successfully. Please click on below link to Login into your account";
                    $signupError = 1;
                } else {
                ...
                $signupError = 1;
                print json_encode(array('status' => 'error', 'msg' => "Invalid signup data."));
                die();
            }
            print json_encode(array('status' => 'success', 'msg' => $succsMsg));
            die();
        } else {
            print json_encode(array('status' => 'error', 'msg' => ""));
            die();
        }
    }
