...
class EventController extends AbstractActionController {
    ...
    private function insertZoneSeats($zoneSeats, $eventId, $zoneId) {
        ...
        foreach ($scheduleData as $data) {
            ...
            if (!empty($zoneSeats)) {
                try {
                    for ($n = 0; $n < count($zoneSeats); $n++) {
                        $match = [];
                        // Save Zone Seats
                        $seatLabel = $zoneSeats[$n]->seatID;
     ...
     * @return ViewModel
     */
    public function blockreleaseAction() {
        $this->layout()->pageTitle = 'Block Or Release';
        $em = $this->getEntityManager(); /* Call Entity Manager */
    ...
    /**
     * ajax change seat status Action
     */
    public function ajaxchangeseatstatusAction() {
        $request = $this->getRequest();  /* Fetching Request */
        $em = $this->getEntityManager(); /* Call Entity Manager */
        if ($request->isPost()) {
            $option = TRUE;
            ...
            if ($status === 'true') {//if true
                $zoneSeatsObj = $em->getRepository('Admin\Entity\ZoneSeats')->findOneBy(array('id' => $seatId, 'seatAvailability' => 0));
            ...
            if ($status === 'false') {
                $zoneSeatsObj = $em->getRepository('Admin\Entity\ZoneSeats')->findOneBy(array('id' => $seatId));
                if (!empty($zoneSeatsObj)) {
                    $zoneSeatsObj->setSeatAvailability(0);
                    $em->persist($zoneSeatsObj);
                    $em->flush();
            ...
            print json_encode(array('status' => 'success', 'option' => $option));
            die();
        } else {
            print json_encode(array('status' => 'error'));
            die();
    ...
    /**
     * ajax block or release seats
     */
    public function ajaxblockreleaseseatsAction() {
        $admin_session = new Container('admin');
        $request = $this->getRequest();  /* Fetching Request */
        $em = $this->getEntityManager(); /* Call Entity Manager */
        if ($request->isPost()) {
            ...
            foreach ($frmData as $value) {
                $seatId = $value['value'];
                $zoneSeatsObj = $em->getRepository('Admin\Entity\ZoneSeats')->findOneBy(array('id' => $seatId));
                if (!empty($zoneSeatsObj)) {
                    $zoneSeatsObj->setSeatAvailability($status);
                    $zoneSeatsObj->setUserId($userId);
                    $em->persist($zoneSeatsObj);
                    $em->flush();
                } else {
                    $option = FALSE;
    ...
    /**
     * ajax get area zone Action
     */
    public function ajaxgetareazoneAction() {
        $request = $this->getRequest();  /* Fetching Request */
        $em = $this->getEntityManager(); /* Call Entity Manager */
        if ($request->isGet()) {
            $eventId = $this->params()->fromQuery('event_id');
            $eventScheduleObj = $em->getRepository('Admin\Entity\EventSchedule')->findBy(array('eventId' => $eventId, 'isDeleted' => 0));
            $scheduleList = array();
            foreach ($eventScheduleObj as $row) {
                $time = $row->getEventTime()->format('h:i A');
                $date = $row->getEventDate()->format('F d, Y');
                $dateTime = $date . ' - ' . $time;
                array_push($scheduleList, array('id' => $row->getId(), 'dateTime' => $dateTime));
            }
            $mapZoneObj = $em->getRepository('Admin\Entity\MapZone')->findBy(array('eventId' => $eventId));
            $areaList = array();
            foreach ($mapZoneObj as $row) {
                array_push($areaList, array('id' => $row->getId(), 'zoneTitle' => ucwords($row->getZoneTitle())));
            }
            print json_encode(array('status' => 'success', 'areaList' => $areaList, 'scheduleList' => $scheduleList));
            die();
        } else {
            print json_encode(array('status' => 'error'));
            die();
    ...
    /**
     * ajax check availability Action
     */
    public function ajaxcheckavailabilityAction() {
        $request = $this->getRequest();  /* Fetching Request */
        $em = $this->getEntityManager(); /* Call Entity Manager */
        if ($request->isPost()) {
            $data = $request->getPost();
            ...
            $zoneId = $data['zoneId'];
            $availability = 2;
            if ($method === 'block') {//to block get free seats
                $availability = 0;
            }
            $zoneSeatsObj = $em->getRepository('Admin\Entity\ZoneSeats')->findBy(array(
            ...
            foreach ($zoneSeatsObj as $row) {
                ...
                $tmpArray['id'] = $row->getId();
                $tmpArray['label'] = $row->getSeatLabel();
                $tmpArray['userId'] = $userId;
                $tmpArray['bookingId'] = $bookingId;
                if ($userId !== 0) {
                    $usersObj = $em->getRepository('Admin\Entity\Users')->find($userId);
                    $tmpArray['userName'] = $usersObj->getFirstName();
                    $tmpArray['userEmail'] = $usersObj->getEmail();
                }
                array_push($seatsArray, $tmpArray);
            }
            $result = $this->buildBlockReleaseHTMLOutput($method, $seatsArray);
    ...
    /**
     * build Block Release HTML Output
     * @param type $method
     * @param type $seatsArray
     * @return string html
     */
    private function buildBlockReleaseHTMLOutput($method, $seatsArray) {
        $title = 'Available seats to block';
        $status = FALSE;
        if ($method !== 'block') {
            $title = 'Blocked seats to release';
            $status = TRUE; //true if method is to release
        }
        $html = '';
        $html .= '<h4 class="section-title">' . $title . '</h4>'
                . '<div class="clear"><div class="col-md-12">';
        if (!empty($seatsArray)) {
            $html .= '<form id="frmBlockRelease" name="frmBlockRelease" class="form-horizontal">';
            $c = 0;
            foreach ($seatsArray as $data) {
                if ($c % 10 == 0) {
                    if ($c === 0) {
                        $html .= '<fieldset id="seats_' . $c . '" class="seatRow">';
                    } else {
                        $html .= '</fieldset><fieldset id="seats" class="seatRow">';
                    }
                    $html .= '<span class="pull-left">'
                            . '<input type="checkbox" class="checkAll" id="start_' . $data["id"] . '" value="0"><br>'
                            . '<label for="start_' . $data["id"] . '">Check Row</label></span>';
                }
                $html .= '<span id="span_' . $data["id"] . '" class="seats seat_' . $method . ' ">';
                $html .= '<label for="seat_' . $data["id"] . '">' . explode('_', $data["label"])[1] . '</label>';
//            $html .= '<input type="radio" class="radioSeat" id="seat_' . $data["id"] . '" name="seat_' . $data["id"] . '" value="' . $data["id"] . '"/>';
                if ($status) {
                    $html .= '<input type="radio" class="radioSeat" id="seat_' . $data["id"] . '" name="seat_' . $data["id"] . '" value="' . $data["id"] . '" checked="true"/>';
                    $html .= '<span class="status"> blocked by ' . $data["userName"] . ' <br/>' . $data["userEmail"] . '</span>';
                } else {
                    $html .= '<input type="radio" class="radioSeat" id="seat_' . $data["id"] . '" name="seat_' . $data["id"] . '" value="' . $data["id"] . '"/>';
                    $html .= '<span class="status"> now available </span>';
                }
                $html .= '</span>';
                $c ++;
            }
            ...
            $html .= '<div class="clear">'
                    . '<div class="col-md-6">'
                    . '<label class="control-label" for="this">Block / Release</label>'
                    . '<div class="clear"></div>'
                    . '<button id="' . $method . '" class="btn btn-primary blockRelease">' . ucwords($method) . ' selected</button>'
                    . '<br><span class="error"></span>'
                    . '</div><div class="col-md-6"></div></div>';
        } else {
            $html .= '<h4 class="section-title">There is no seat to ' . $method . '</h4>';
        }
        return $html;
    }
     ...
     * @return string 
     */
    private function getEventReport($eventObj, $eventId) {
        $em = $this->getEntityManager(); /* Call Entity Manager */
        $eventName = $eventObj->getEventName();
        $eventDesc = $eventObj->getEventDesc();
        $eventZip = $eventObj->getEventZip();

        $userBookingObj = $em->getRepository('Admin\Entity\UserBooking')->findBy(array('event' => $eventId, 'status' => 1));

        $html = '<div class="row section-title">'
                . '<div class="col-md-6">'
                . '<h4>' . $eventName . '<br><small>' . $eventDesc . '</small></h4>'
                . '</div><div class="col-md-6">'
                . '<button id="printThis" class="btn btn-primary rgtbn pull-right">Print</button>'
                . '</div></div>';
        $html .= '<div class="row"><div class="table-responsive"><table class="table table-striped"><thead><tr><th>Purchased DateTime</th><th>Ticket#</th><th>Payment Type</th><th>Amount</th><th>First Name</th><th>Last name</th><th>Quantity</th><th>Email</th><th>Zip code</th><th>Contact No:</th><th>Show date</th><th>Zone</th><th>Seat</th></tr></thead><tbody>';
        $total = 0;
        foreach ($userBookingObj as $row) {
            ...
            $bookingId = $row->getId();
            $getUser = $row->getUser();
            $userObj = $em->getRepository('Admin\Entity\Users')->findOneBy(array('id' => $getUser, 'status' => 1));
            $userId = $userObj->getId();
            $count = $row->getBookingSeatCount();
            $tmpLable = '';
            $tmpZone = '';
            for ($i = 0; $i < $count; $i++) {
                $zoneSeatsObj = $em->getRepository('Admin\Entity\ZoneSeats')->findBy(array('eventId' => $eventId, 'bookingId' => $bookingId, 'userId' => $userId, 'seatAvailability' => 2));
                $zoneId = $zoneSeatsObj[$i]->getZoneId();
                $mapZoneObj = $em->getRepository('Admin\Entity\MapZone')->findOneBy(array('id' => $zoneId));
                if ($tmpLable === '') {
                    $tmpZone = $mapZoneObj->getZoneTitle();
                    $tmpLable = explode('_', $zoneSeatsObj[$i]->getSeatLabel())[1];
                } else {
                    $seatLabel = $tmpLable . ',' . explode('_', $zoneSeatsObj[$i]->getSeatLabel())[1];
                    $zoneTitle = $tmpZone . ', ' . $mapZoneObj->getZoneTitle();
                }
            }
            $total = (intval($total)) + (intval($row->getBookingTotalPrice()));
            $html .= '<tr>';
            ...
            $html .= '<td>' . $zoneTitle . '</td>';
            $html .= '<td>' . $seatLabel . '</td>';
            $html .= '</tr>';
        }
        $html .= '</tbody></table></div></div>';
        $html .= '<div class="row section-title">'
                . '<div class="col-md-6"><h4>Event Total Ticket Sold (AED):<span class="pull-right">' . $total . '</span></h4></div>'
...
//        die();
     ...
     * @return type
     */
    private function buildReportHTMLOutput($data) {
        $html = '';
        $eventId = '';
        $startdate = '';
        $enddate = '';
        ...
        if (isset($data['eventlist'])) {
            $eventId = $data['eventlist'];
            $eventObj = $em->getRepository('Admin\Entity\Event')->findOneBy(array('id' => $eventId, 'status' => 1));
            $html .= $this->getEventReport($eventObj, $eventId);
        }
        if (isset($data['startdate'])) {
            $startdate = $data['startdate'];
            $enddate = $data['enddate'];
            $eventScheduleObj = $em->getRepository('Admin\Entity\EventSchedule')->getEventScheduleBetweenDates('2016-01-01', '2016-01-30');
        ...
        if ($request->isPost()) {
            ...
            switch ($reporttype) {
                case 'fnsreport':
                    break;
                case 'eventlist':
                    $result = $this->buildReportHTMLOutput($data);
                    break;
                case 'startend':
                    $result = $this->buildReportHTMLOutput($data);
                    break;
