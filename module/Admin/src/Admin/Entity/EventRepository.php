...
class EventRepository extends EntityRepository {
    ...
    private function getAvailableSeatCountByEventId($eventId){
        $em = $this->getEntityManager();
        $query = $em->createQueryBuilder('u');
        $query = $query->from('Admin\Entity\ZoneSeats', 'u');
        $query = $query->select('count(u)');
        $query = $query->where('u.eventId = :eventId')->setParameter('eventId', $eventId);
        $query = $query->andWhere('u.seatAvailability != 0');
    ...
    public function getEventsListingAtAdminForDataTable($sqlArr) {
        ...
        foreach ($Result as $eventRow) {
            $id = $eventRow->getId();
            $countArray = $this->getAvailableSeatCountByEventId($id)[1];
            $count = intval($countArray);
...
//		print_r($query->getSql());die();
     ...
     * @author Aditya
     */
    public function getDateEvent($type = null, $val = null, $offset = null, $limit = null) {
        $em = $this->getEntityManager();
...
//      $query = $query->andWhere("es.isDeleted = 0");
        if ($type == 'month'){
            ...
            $query = $query->andWhere("YEAR(s.eventDate) = '".$explode_val[0]."'");
            $query = $query->andWhere("MONTH(s.eventDate) = '".$explode_val[1]."'");
        }
        else if ($type == 'weekday')
            $query = $query->andWhere("s.eventDate = '" . $val . "'");
        else
            $query = $query->andWhere("s.eventDate = CURRENT_DATE()");
...
//        $query = $query->andWhere("o.isDeleted = 0");
    ...
    public function getHomePageEvents($cityobj = "") {
        ...
        if (!empty($cityobj)) {
			...
			// added change by Shehan@ITSthe1 - 28012016
			if(gettype($cityobj) == "array"){
				for($c=0; $c<count($cityobj); $c++){
					if($c==0){
						$query = $query->where("e.eventCity = " . $cityobj[$c]->getId());
					} else {
						$query = $query->orWhere("e.eventCity = " . $cityobj[$c]->getId());
					}
				}
			} else if(gettype($cityobj) == "object") {
				$query = $query->andWhere("e.eventCity = " . $cityobj->getId());
			}
			// end change
        }
